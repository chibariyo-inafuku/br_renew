...
function br_enqueue_assets() {
	...
	if ( is_singular( 'post' ) ) {
		...
		$post_single_content_deps = array( 'br-home' );
		if ( $post_id > 0 && function_exists( 'br_post_in_blog_category_tree' ) && br_post_in_blog_category_tree( $post_id ) ) {
			...
			wp_enqueue_style(
				'br-related-rail',
				$theme_uri . '/assets/css/related-rail.css',
				array( 'br-post-single', 'br-swiper' ),
				BR_VERSION
			);
			wp_enqueue_script(
				'br-swiper',
			...
			$post_single_content_deps = array( 'br-service-single' );
			...
			$post_single_content_deps = array( 'br-news-single' );
		}
		wp_enqueue_style(
			'br-single-content',
			$theme_uri . '/assets/css/single-content.css',
			$post_single_content_deps,
			BR_VERSION
		);
	}
	...
	if ( is_singular( 'portfolio' ) ) {
		...
		wp_enqueue_style(
			'br-related-rail',
			$theme_uri . '/assets/css/related-rail.css',
			array( 'br-portfolio-single' ),
			BR_VERSION
		);
		wp_enqueue_style(
			'br-single-content',
			$theme_uri . '/assets/css/single-content.css',
			array( 'br-related-rail' ),
			BR_VERSION
		);
		wp_enqueue_script(
			'br-swiper',
